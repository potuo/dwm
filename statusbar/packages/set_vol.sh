...
send_notification() {
  ...
  if is_mute; then
    ...
    dunstify -a "volume" -u low -i $icon -h string:x-dunst-stack-tag:volume "音量: 静音"
  else
    dunstify -a "volume" -u low -i $icon -h int:value:"$volume" -h string:x-dunst-stack-tag:volume "音量: $volume%"
  fi
}
